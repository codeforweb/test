...
$(function() {
    ...
    // adding condition to intilialize floating bar only when it exists
    ...
    $('.sticky-nav-links').on('click', function(e){
        ...
        // adding animation for scrolling
        /* Need to refactor this because there are links to pages and not just anchor links.
        e.preventDefault();
    ...
    $('.expand-rows').on('click', '.expand', function(e)
    {
        e.preventDefault();

        var t = $(e.target),
        section = t.attr('id').substring(7);

        // Toggle open classes
        $(t).toggleClass('open');
        $('#para-' + section).toggleClass('open');
    });
